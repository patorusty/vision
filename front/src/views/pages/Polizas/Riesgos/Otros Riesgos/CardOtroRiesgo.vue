<template>
  <v-card class=" mx-4 pa-3">
    ...
    <v-card-text>
      <v-row v-if="poliza.tipo_riesgo_id == 3 || poliza.tipo_riesgo_id == 4 || poliza.tipo_riesgo_id == 5 || poliza.tipo_riesgo_id == 10 || poliza.tipo_riesgo_id == 9 || poliza.tipo_riesgo_id == 6 || poliza.tipo_riesgo_id == 11|| poliza.tipo_riesgo_id == 12">
        <v-col cols="4">
          <v-text-field
            label="Actividad"
            v-model="otro_riesgo.actividad"
            v-uppercase
          >
        ...
        <v-row>
          ...
          <v-col class="pt-0 pb-0">
            <v-autocomplete
              ...
              item-value="id"
              v-model="otro_riesgo.localidad_id"
              label="Localidad"
            ></v-autocomplete>
          </v-col>
        </v-row>
      ...
      <v-row>
        ...
        <v-col v-if="poliza.tipo_riesgo_id == 8">
          <v-text-field
            label="Tipo de embarcacion"
            v-model="otro_riesgo.tipo_embarcacion"
            v-uppercase
          > </v-text-field>
        </v-col>
        <v-col v-if="poliza.tipo_riesgo_id == 8">
        ...
        <v-col v-if="poliza.tipo_riesgo_id == 7 || poliza.tipo_riesgo_id == 8 || poliza.tipo_riesgo_id == 9">
          <v-text-field
            ...
            v-model="otro_riesgo.suma_asegurada"
            v-uppercase
          > </v-text-field>
        </v-col>
    ...
    <v-card-actions class="mt-0 py-0 d-flex justify-end">
      <v-btn
        v-if="poliza.otro_riesgo && Object.keys(poliza.otro_riesgo).length > 0"
        class="mb-2"
        color="green"
        text
        @click="update"
